...
namespace Shoe_Store_DB.DA_Layer
{
    internal class ProductQuantityDA
    {
        ...
        {
            string query = "SELECT pq.product_quantity_id, p.product_name, s.size_name as product_size, c.color_name as product_color, pq.product_quantity  FROM shoe_store_db.product_quantity pq join product p using(product_id) join size s using(size_id) join color c using(color_id) where pq.product_id = @firstParameter order by 1;";
            ...
            if (cmd != null)
            {
                ...
                foreach (DataRow dr in dt.Rows)
                {
                    ...
                    ProductQuantity productQuantity = new ProductQuantity(id, name, size, color, quantity);
