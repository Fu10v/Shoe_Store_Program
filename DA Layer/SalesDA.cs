...
namespace Shoe_Store_DB.DA_Layer
{
    internal class SalesDA
    {
        ...
        {
            string query = "SELECT s.sales_id, concat(e.employee_first_name, ' ', e.employee_surname, ' ', e.employee_middle_name) as employee, ifnull(concat(c.customer_first_name, ' ', c.customer_surname, ' ', c.customer_middle_name), '') as customer, s.sale_time, ifnull(group_concat(Distinct p.product_name order by 1 separator ', '), '') as sold_items, ifnull(sum(sales_list_quantity), 0) as quantity, ifnull(sum(product_price * sales_list_quantity), 0) as total from sales s join employee e using(employee_id) left join customer c using(customer_id) left join sales_list using(sales_id) left join product_quantity using(product_quantity_id) join product using(product_id) where concat(e.employee_first_name, ' ', e.employee_surname, ' ', e.employee_middle_name) like @searchParameter or ifnull(concat(c.customer_first_name, ' ', c.customer_surname, ' ', c.customer_middle_name), '') like @searchParameter or sale_time like @searchParameter or p.product_name like @searchParameter group by 1 order by sale_time desc;";
